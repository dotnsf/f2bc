...
const HyperledgerClient = function() {
  ...
  vm.prepare = ( resolved, rejected ) => {
    if( vm.businessNetworkConnection != null && vm.businessNetworkDefinition != null ){
      ...
      console.log('HyperLedgerClient.prepare(): import card');
      var IdCard = require('composer-common').IdCard;
      var AdminConnection = require('composer-admin').AdminConnection;
      var fs = require('fs');
      var adminConnection = new AdminConnection();
      var card_id = settings.cardName; //'admin@myvc-network';
      adminConnection.hasCard(card_id).then(hasCard => {
        if( !hasCard ){
          let data = fs.readFileSync( './admin@myvc-network.card' );
          return IdCard.fromArchive( data );
        }
      }).then( card => {
        if( card != null ){
          return adminConnection.importCard( card_id, card );
        }
      }).then( result => {
        if( result != null ){
          console.log( card_id + ' card is imported successfully.' );
        }
      }).catch( err => {
        console.log( err );
      });
